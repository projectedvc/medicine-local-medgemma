...
def get_report(
    ...
    if not report:
        raise HTTPException(status_code=404, detail="Заключение еще не создано")
    return report
...
def create_draft(
    ...
    analysis = (
        db.query(AIAnalysis)
        .filter(AIAnalysis.study_id == study.id, AIAnalysis.status == AIJobStatus.completed)
        .order_by(AIAnalysis.created_at.desc())
        .first()
    )
    report = _get_or_create_report(db, study.id)
    report.ai_draft_text = build_ai_draft(study, analysis)
...
def export_pdf(
    ...
    path = export_report_pdf(study, report, doctor)
...
def export_docx(
    ...
    study, report, doctor = _export_guard(db, current_user, study_id)
    path = export_report_docx(study, report, doctor)
    study.status = StudyStatus.exported
    db.commit()
